...
- Varje task måste ha CLI-test och max 150 LOC
...
VESIR_TOOLS = [
    ...
    {
        "name": "create_subtask",
        "description": "Skapa en atomär subtask. Varje task måste ha CLI-test och vara max 150 LOC.",
        "input_schema": {
            "type": "object",
            "properties": {
                "description": {
                ...
                "estimated_loc": {
                    "type": "integer",
                    "description": "Uppskattad antal rader kod (max 150)"
                },
                "dependencies": {
                    "type": "array",
                    "items": {"type": "string"},
            ...
            "required": ["description", "cli_test", "estimated_loc"]
        }
    },
    {
        "name": "reject_request",
...
1. Max {MAX_LOC} rader kod (estimated_loc)
...
- "Bygg en webbserver med autentisering" (för stort, ingen CLI-test)
- "Fixa buggen" (otydligt, ingen specifikation)"""
...
Skapa sedan atomära tasks med tydliga kontrakt."""
    ...
    def execute_tool(self, name: str, arguments: dict) -> dict:
        ...
        if name == "list_files":
            ...
            # Validera estimated_loc
            loc = arguments.get("estimated_loc", 0)
            if loc > MAX_LOC:
                return {
                    "success": False,
                    "error": f"Task är för stor ({loc} LOC). Max är {MAX_LOC}. Bryt ner ytterligare."
                }
