...
def get_dev_token(db: Session = Depends(get_db)):
    """Get a dev token (creates test user if needed) - FOR DEVELOPMENT ONLY"""
    auth_service = AuthService(db)
    tenant_service = TenantService(db)
    ...
    if not user:
        # Create dev tenant
        tenant = tenant_service.get_by_slug("dev-team")
        if not tenant:
            tenant = tenant_service.create(name="Dev Team", slug="dev-team")
