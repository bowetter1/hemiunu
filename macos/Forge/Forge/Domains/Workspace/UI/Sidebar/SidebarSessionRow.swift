...
struct SidebarSessionRow: View {
    ...
    var onFork: ((LocalProject) -> Void)? = nil
    var onDeleteVersion: ((LocalProject) -> Void)? = nil
    ...
    private var multiProjectGroup: some View {
        VStack(spacing: 0) {
            HStack(spacing: 0) {
                ...
                if isExpanded && (!isSelected(group) || group.projects.count == 1) {
                    Menu {
                        if let last = group.projects.last {
                            Button(action: { onFork?(last) }) {
                    ...
                    ForEach(Array(group.projects.enumerated()), id: \.element.id) { index, project in
                        VStack(spacing: 0) {
                            HStack(spacing: 0) {
                                ...
                                if isSelected(project) && group.projects.count > 1 {
                                    Menu {
                                        Button(action: { onFork?(project) }) {
                                            Label("Fork", systemImage: "arrow.triangle.branch")
                                        }
                                        Divider()
                                        Button(role: .destructive, action: { onDeleteVersion?(project) }) {
