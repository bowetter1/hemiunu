...
struct BuildSiteSheet: View {
    ...
    private func addCustomPage() {
        ...
        // Don't add if it's already a nav link
    ...
    private func parseNavLinks() {
        ...
        // Pre-select all nav links
        selectedLinks = Set(links.map(\.name))
    }

    private func readCurrentHTML() -> String? {
        // Prefer page HTML already loaded in appState (always up to date)
        if let page = appState.pages.first(where: { $0.name == "index.html" }),
           !page.html.isEmpty {
            return page.html
        }
        // Fallback: any page without a layoutVariant
    ...
    private func buildSite() {
        ...
        // Note which pages are anchor-only (sections on the same page)
...
private struct NavLink {
    let name: String
    let href: String?
    /// True if the href is an anchor (#about) — a section on the page, not a separate file
    let isAnchor: Bool
}
